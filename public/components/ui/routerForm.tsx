...
interface RouterFormProps {
    ...
    type: 'UPDATE' | 'CREATE';
    createRouter: (data: RouterData) => void;
    updateRouter: (data: RouterData) => void;
    router?: RouterData;
}

const RouterForm: React.FC<RouterFormProps> = ({ showForm, setShowForm, type, router, createRouter, updateRouter }) => {
    const { t } = useTranslation();
    const { register, setValue, handleSubmit, reset } = useForm<RouterData>();
    ...
    useEffect(() => {
        if (showForm && router && type === 'UPDATE') {
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className='flex flex-col gap-2 p-6 pb-2'>
                                <h4 className='block font-sans text-2xl antialiased font-semibold leading-snug tracking-normal text-blue-gray-900'>
                                    {type == 'CREATE' ? t('GENERIC_CREATE_ROUTER') : t('GENERIC_UPDATE_ROUTER')}
                                </h4>
